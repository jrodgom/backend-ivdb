import requests
from .models import Game

RAWG_API_URL = "https://api.rawg.io/api/games"
RAWG_API_KEY = "3994c5d1391e4fc8b4e7eca9428d8a8b"  # ⚠️ NO subir a GitHub

def import_games_from_rawg(page_size=10):
    """Importa juegos desde RAWG y los guarda en la base de datos."""
    params = {
        "key": RAWG_API_KEY,
        "page_size": page_size,  # cuántos juegos quieres traer
    }

    response = requests.get(RAWG_API_URL, params=params)

    if response.status_code != 200:
        print("Error al conectar con RAWG:", response.status_code)
        return

    data = response.json()
    results = data.get("results", [])

    for game_data in results:
        title = game_data["name"]
        cover_image = game_data["background_image"]
        release_date = game_data["released"]
        developer = None  # RAWG no siempre lo trae
        genre = ", ".join([g["name"] for g in game_data.get("genres", [])])
        platform = ", ".join([p["platform"]["name"] for p in game_data.get("platforms", [])])

        # Evita duplicados
        if not Game.objects.filter(title=title).exists():
            Game.objects.create(
                title=title,
                description="Importado desde RAWG",
                release_date=release_date if release_date else None,
                genre=genre,
                platform=platform,
                developer=developer if developer else "",
                cover_image=cover_image if cover_image else "",
            )
            print(f"✅ Añadido: {title}")
        else:
            print(f"⏩ Ya existía: {title}")
